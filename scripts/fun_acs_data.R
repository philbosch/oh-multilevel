clean_acs_data <- function(acs_vars_file, year = 2012) {
  ...
  acs_bg <- acs_raw_bg %>%
    ...
    dplyr::mutate(tract = stringr::str_sub(GEOID, start = 1, end = -2)) %>%
    dplyr::select(GEOID, tract, tidyselect::starts_with("bg_")) %>%
    dplyr::mutate(dplyr::across(tidyselect::starts_with("bg_"), ~ifelse(is.nan(.), 0, .)))
  ...
  acs_tract <- acs_raw_tract %>%
    ...
    dplyr::select(GEOID, tidyselect::starts_with("tr_")) %>%
    dplyr::mutate(dplyr::across(tidyselect::starts_with("tr_"), ~ifelse(is.nan(.), 0, .)))
  ...
  acs_clean <- acs_bg %>%
    dplyr::left_join(acs_tract, by = dplyr::join_by(tract == GEOID))
